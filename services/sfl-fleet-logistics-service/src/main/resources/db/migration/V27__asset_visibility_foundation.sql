...
-- The owning deployable is `sfl-fleet-logistics-service`, not the retired
-- `sfl-asset-visibility-service`. The row names the process that runs the migrations, so leaving the
-- old name would have this schema assert a service nobody can start. The bounded context is
-- unchanged: AVAMP-Lite still owns this schema alone, and no fleet, fuel or dispatch table may carry
-- a foreign key into it.
INSERT INTO asset_visibility.service_metadata (service_name, boundary_description)
VALUES ('sfl-fleet-logistics-service', 'AVAMP-Lite asset and device reference layer, and future S168')
