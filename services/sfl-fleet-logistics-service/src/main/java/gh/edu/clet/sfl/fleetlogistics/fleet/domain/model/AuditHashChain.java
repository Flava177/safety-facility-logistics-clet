...
/**
 * The tamper-evident audit hash chain required by SRS-SFL-S166-03
 * ("Audit records shall be append-only and tamper-evident using a hash-chain or equivalent control").
 *
 * <p>{@code hash = SHA-256(previousHash ‖ canonical(record))}. The canonical form is a fixed field order
 * with a unit separator that cannot appear in the field values, so two different records can never
 * canonicalise to the same string.
 *
 * <p>Replaying the chain detects a mutated record (its hash no longer matches), a removed record (the
 * sequence gaps and the link breaks) and a reordered record (the previous-hash link breaks).
 */
public final class AuditHashChain {
    ...
    /** ASCII unit separator: cannot occur in any audit field value, so canonical forms are unambiguous. */
    private static final char UNIT_SEPARATOR = '\u001F';
    /** ASCII record separator: distinguishes an absent field from an empty one. */
    ...
    public static AuditEvent seal(AuditEvent event, long sequenceNo, String previousHash) {
        ...
        String previous = previousHash == null || previousHash.isBlank() ? GENESIS_HASH : previousHash;
    ...
    public static String computeHash(String previousHash, AuditEvent event) {
        Objects.requireNonNull(event, "event is required");
        String previous = previousHash == null || previousHash.isBlank() ? GENESIS_HASH : previousHash;
        return sha256Hex(previous + UNIT_SEPARATOR + canonical(event));
    }
    ...
    public static AuditChainVerification verify(List<AuditEvent> orderedEvents, String expectedFirstPreviousHash) {
        ...
        String previousHash = expectedFirstPreviousHash == null || expectedFirstPreviousHash.isBlank()
                ? GENESIS_HASH
                : expectedFirstPreviousHash;
        ...
        for (AuditEvent event : orderedEvents) {
            if (event.sequenceNo() != expectedSequence) {
                return AuditChainVerification.broken(expectedSequence, event.sequenceNo(),
                        "Audit sequence is not contiguous", orderedEvents.size());
            }
            if (!previousHash.equals(event.previousHash())) {
                return AuditChainVerification.broken(event.sequenceNo(), previousHash, event.previousHash(),
                        "Previous-hash link does not match the preceding record", orderedEvents.size());
            }
            ...
            if (!recomputed.equals(event.recordHash())) {
                return AuditChainVerification.broken(event.sequenceNo(), recomputed, event.recordHash(),
                        "Record hash does not match the stored content", orderedEvents.size());
            }
            previousHash = event.recordHash();
            expectedSequence++;
        }
        return AuditChainVerification.intact(orderedEvents.size(), previousHash);
    }
    ...
    private static String canonical(AuditEvent event) {
        ...
        append(builder, event.siteScope().value());
