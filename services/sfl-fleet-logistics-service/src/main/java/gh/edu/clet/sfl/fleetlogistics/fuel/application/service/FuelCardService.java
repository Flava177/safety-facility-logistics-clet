 ...
 * The platform could tell you which card was used and could answer nothing about it: whether the
 * card is ours, whether it is still active, whether it belongs to the vehicle that was filled. The
 * C9 mapping gives S168_fuel "anti-fraud controls" as its purpose, and the commonest fuel fraud is
 * a card assigned to one vehicle filling another - which needs a register to be visible at all.
 *
 * <p><strong>Issuing is a manager's act.</strong> A fuel card is a payment instrument, so {@code
 * FUEL_CARD_MANAGE} sits with {@code FLEET_MANAGER} and {@code SFL_ADMIN}; the logistics officer
 * who runs fuel operations day to day gets {@code FUEL_CARD_READ} and cannot issue one. A driver
 * gets neither - the register says which cards exist and who holds them, which is not a driver's
 * business.
 */
@Service
public class FuelCardService {
    ...
    public FuelCardService(
            FuelRepository repository, FuelAccessPolicy access, AuditPort audit, Clock clock) {
    ...
    public FuelCard issue(IssueCard command) {
        access.require(
                command.actor(),
                SflPermission.FUEL_CARD_MANAGE,
                command.siteCode(),
                "FuelCard",
                null);
        ...
        repository
                .findLiveCardByReference(site.value(), command.maskedReference())
                .ifPresent(
                        existing -> {
                            throw new IllegalStateException(
                                    "A live fuel card already exists for reference "
                                            + command.maskedReference()
                                            + " at "
                                            + site.value()
        ...
        FuelCard card =
                FuelCard.issue(
                        ...
                        command.vehicleId(),
                        command.driverId(),
                        command.issuedOn() == null ? today() : command.issuedOn(),
                        command.expiresOn(),
                        command.dailyLimit(),
                        command.monthlyLimit(),
                        command.perTransactionLimit(),
                        command.notes(),
     ...
     *
     * <p>One entry point rather than five, because every one of these is the same shape - read,
     * check, move, audit - and five near-identical methods is where the audit call gets forgotten
     * on the sixth.
     */
    @Transactional
    public FuelCard transition(TransitionCard command) {
        FuelCard before =
                repository
                        .findCard(command.cardId())
                        .orElseThrow(
                                () -> RecordNotFoundException.of("FuelCard", command.cardId()));
        access.require(
                command.actor(),
                SflPermission.FUEL_CARD_MANAGE,
        ...
        FuelCard after =
                switch (command.action()) {
                    case "assign" -> before.assignTo(
                            command.vehicleId(), command.driverId(), metadata);
                    case "suspend" -> before.suspend(
                            requireReason(command.reason(), "suspend"), metadata);
                    case "reinstate" -> before.reinstate(metadata);
                    case "cancel" -> before.cancel(
                            requireReason(command.reason(), "cancel"), metadata);
                    default -> throw new IllegalArgumentException(
                            "Unknown fuel card action '"
                                    + command.action()
    ...
    @Transactional(readOnly = true)
    public FuelRepository.FuelPage<FuelCard> cards(
            String site,
        ...
        return repository.findCards(
                new FuelRepository.CardQuery(
                        java.util.List.of(SiteCode.of(site).value()),
                        status,
                        vehicleId,
                        driverId,
                        maskedReference,
    ...
    /**
     * Suspending or cancelling somebody's card without a reason is an argument at a filling
     * station.
     */
    private static String requireReason(String reason, String action) {
        if (reason == null || reason.isBlank()) {
            throw new IllegalArgumentException(
                    "A reason is required to " + action + " a fuel card");
        }
        return reason.strip();
    }
