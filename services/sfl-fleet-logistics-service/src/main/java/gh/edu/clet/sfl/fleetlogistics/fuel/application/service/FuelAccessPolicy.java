...
public class FuelAccessPolicy {
    public void require(ActorContext actor,SflPermission permission,String site,String resource,String id){
        if(!FuelPermissionMatrix.grants(actor.principal().roles(),permission)||!actor.principal().canAccessSite(site)) throw new FleetAuthorizationException(Map.of("requiredPermission",permission.name(),"siteCode",site,"resourceType",resource,"resourceId",id==null?"":id));
    }
