...
public record FuelPolicy(
    ...
    public FuelPolicy {
        ...
        Objects.requireNonNull(maxPerTransaction);
        Objects.requireNonNull(materialityAmount);
        Objects.requireNonNull(status);
        Objects.requireNonNull(metadata);
        costVarianceTolerance =
                costVarianceTolerance == null
                        ? DEFAULT_COST_VARIANCE_TOLERANCE
                        : costVarianceTolerance;
        name = require(name, "name");
        if (effectiveTo != null && !effectiveTo.isAfter(effectiveFrom))
            throw new IllegalArgumentException("effectiveTo must follow effectiveFrom");
        if (policyVersion < 1
                || maxPerTransaction.signum() <= 0
                || odometerJumpTolerance < 0
                || receiptGraceHours < 0
                || materialityAmount.signum() < 0
                || anomalySlaHours < 1
                || costVarianceTolerance.signum() < 0
    ...
    public FuelPolicy(
        ...
        this(
                ...
                anomalySlaHours,
                DEFAULT_COST_VARIANCE_TOLERANCE,
                DEFAULT_REPEATED_PATTERN_WINDOW_HOURS,
    ...
    public boolean allowsProduct(String product) {
        return allowedFuelProducts.isEmpty() || allowedFuelProducts.contains(product.toUpperCase());
    }

    public boolean allowsVendor(String vendor) {
        return approvedVendors.isEmpty() || approvedVendors.contains(vendor.toUpperCase());
    }
     ...
     * includes {@code name}, the effective period and the audit metadata - none of which changes
     * how a transaction is judged. Renaming a policy or correcting its description must stay free.
     *
     * <p>The effective period is deliberately excluded too. It decides <em>which</em> policy
     * applies to a transaction, not what that policy does, and a completed run has already recorded
     * its outcome; the overlap rule is what governs the period.
     *
