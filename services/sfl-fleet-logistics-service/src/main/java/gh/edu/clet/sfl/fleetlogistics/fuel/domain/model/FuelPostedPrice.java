 ...
 *
 * <p>Because it is the one number in a fuel transaction that is not the driver's to choose. Litres and
 * cost are both observations of what happened; the price per litre is a fact about the forecourt, the
 * same for every vehicle that fills there that day. Leaving it as a free field made a fuel claim a set
 * of three numbers all supplied by the person being reimbursed, and any two of them determine the
 * third - so there was no arithmetic anyone could do to check the claim against anything.
 *
 ...
 * Fuel prices here move every few weeks; without this, every historical transaction would drift into
 * an anomaly the moment the pumps changed, and reconciliation would stop being reproducible - which is
 * the same reason {@link FuelPolicy} is effective-dated.
 */
public record FuelPostedPrice(UUID id, SiteCode siteCode, String vendor, String fuelProduct,
        BigDecimal unitPrice, String currency, Instant effectiveFrom, Instant effectiveTo, Source source,
     ...
     * kinds of evidence, and once a month has passed nobody can tell them apart from the number alone.
     */
    public enum Source {
    ...
    public FuelPostedPrice {
        ...
        // under "GOIL" and a policy allowing "Goil" would otherwise be two different vendors, and the
        // price lookup would silently find nothing - which reads as "no reference price" rather than
    ...
    public FuelPostedPrice supersededAt(Instant at, RecordMetadata changed) {
        ...
        return new FuelPostedPrice(id, siteCode, vendor, fuelProduct, unitPrice, currency, effectiveFrom,
                at, source, notes, changed);
    }
     ...
     * question as paying more - it is what a receipt for fuel that never went into the tank looks like.
     */
