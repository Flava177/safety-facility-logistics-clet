...
@RestController
@RequestMapping("/api/v1/fuel/anomalies")
...
public class FuelAnomalyController {
    ...
    public FuelAnomalyController(FuelApplicationService s, FleetActorResolver a) {
        service = s;
        actors = a;
    }
     ...
     */
    @GetMapping
    public ApiResponse<FuelPageResponse<FuelAnomalyCase>> list(
    ...
    @GetMapping("/{id}")
    public ApiResponse<FuelAnomalyCase> detail(@PathVariable UUID id, HttpServletRequest h) {
        return ApiResponse.ok(service.anomaly(id, actors.resolve(h)));
    }
    ...
    @PostMapping(
            "/{id}/{action:assign|reassign|review|request-explanation|explain|approve|reject|escalate|hold|resume|cancel|close|reopen}")
    public ApiResponse<FuelAnomalyCase> transition(
