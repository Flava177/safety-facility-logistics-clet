...
@RestController
@RequestMapping("/api/v1/fuel/logbooks")
...
public class DriverLogbookController {
    ...
    public DriverLogbookController(FuelApplicationService s, FleetActorResolver a) {
        service = s;
        actors = a;
    }

    @PostMapping
    public ResponseEntity<ApiResponse<DriverLogbook>> create(
            @Valid @RequestBody LogbookRequest r, HttpServletRequest h) {
        var l =
                service.createLogbook(
                        new FuelApplicationService.CreateLogbook(
                                r.siteCode(),
                                r.driverId(),
                                r.vehicleId(),
                                r.tripId(),
                                ...
                                r.useClassification(),
                                r.purpose(),
                                r.passengerLoadNotes(),
                                r.startOdometer(),
                                r.endOdometer(),
                                r.declarationAccepted(),
                                r.evidenceId(),
                                actors.resolve(h),
                                actors.resolveSourceChannel(h)));
        return ResponseEntity.created(URI.create("/api/v1/fuel/logbooks/" + l.id()))
                .body(ApiResponse.ok(l));
    }

    @PatchMapping("/{id}")
    public ApiResponse<DriverLogbook> update(
            @PathVariable UUID id, @Valid @RequestBody LogbookRequest r, HttpServletRequest h) {
        return ApiResponse.ok(
                service.updateLogbook(
                        id,
                        new FuelApplicationService.UpdateLogbook(
                                r.driverId(),
                                r.vehicleId(),
                                r.tripId(),
                                r.journeyDate(),
     ...
     */
    @GetMapping
    public ApiResponse<FuelPageResponse<DriverLogbook>> list(
    ...
    @GetMapping("/{id}")
    public ApiResponse<DriverLogbook> detail(@PathVariable UUID id, HttpServletRequest h) {
        return ApiResponse.ok(service.logbook(id, actors.resolve(h)));
    }
    ...
    @PostMapping("/{id}/{action:submit|review|return|approve|reopen|cancel}")
    public ApiResponse<DriverLogbook> transition(
            @PathVariable UUID id,
            @PathVariable String action,
            @RequestBody(required = false) TransitionRequest r,
            HttpServletRequest h) {
        return ApiResponse.ok(
                service.transitionLogbook(
                        id,
                        action,
                        r == null ? null : r.comment(),
                        actors.resolve(h),
                        actors.resolveSourceChannel(h)));
    }
    ...
    public record LogbookRequest(
            ...
            String routeNotes,
            @NotNull DriverLogbook.UseClassification useClassification,
            @NotBlank String purpose,
            String passengerLoadNotes,
            @PositiveOrZero long startOdometer,
            Long endOdometer,
