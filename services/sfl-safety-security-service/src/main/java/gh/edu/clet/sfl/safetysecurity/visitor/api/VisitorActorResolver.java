...
/**
 * Produces the {@link ActorContext} every visitor-management command/query is authorised against,
 * from the OIDC/JWT principal in production or the {@code X-SFL-*} development headers otherwise.
 * Mirrors {@code EmergencyActorResolver} in the sibling S174 module rather than reusing it directly -
 * that class is emergency-scoped and not shared, the same way each module in this service keeps its
 * own copy of small platform-wide plumbing.
 */
@Component
public class VisitorActorResolver {
    ...
    private static Set<SflRole> parseRoles(String header) {
        return parseCsv(header).stream().map(VisitorActorResolver::toRole)
                .filter(Objects::nonNull).collect(Collectors.toUnmodifiableSet());
    }
