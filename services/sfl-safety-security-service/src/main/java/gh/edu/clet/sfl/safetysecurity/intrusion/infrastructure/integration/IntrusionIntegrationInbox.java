...
/**
 * Secure inbound-message intake (SRS-SFL-S162-01): source allowlist, HMAC signature + timestamp
 * window, schema validation and idempotency, persisting the inbox envelope BEFORE any domain side
 * effect. Mirrors {@code accesscontrol.infrastructure.integration.AccessControlIntegrationInbox}
 * exactly, with the allowlist/secret drawn from {@code sfl.intrusion.*} properties for the same
 * reason that module gives: a static allowlist is a real feature this module can grow into once a
 * real panel vendor and monitoring feed exist to configure it with.
 */
@Component
public class IntrusionIntegrationInbox {
    ...
    public IntrusionIntegrationInbox(JdbcTemplate jdbc, Clock clock,
            @Value("${sfl.intrusion.integration.allowed-sources:INTRUSION-PANEL-VENDOR,MONITORING-SERVICE}")
            List<String> allowlistedSources,
            @Value("${sfl.intrusion.integration.shared-secret:dev-only-not-a-real-secret}") String sharedSecret) {
        this.jdbc = jdbc;
        this.clock = clock;
    ...
    public UUID accept(InboundMessage message) {
        ...
        if (message.idempotencyKey() == null || message.idempotencyKey().isBlank()) {
            throw new IntrusionException(IntrusionErrorCode.INTRUSION_SCHEMA_VALIDATION_FAILED,
                    Map.of("field", "idempotencyKey"));
        }
        Long existing = jdbc.queryForObject("""
                SELECT COUNT(*) FROM safety_security.intrusion_inbox_messages
                WHERE source_system=? AND idempotency_key=?
                """, Long.class, source, message.idempotencyKey());
        if (existing != null && existing > 0) {
            throw new IntrusionException(IntrusionErrorCode.INTRUSION_DUPLICATE_MESSAGE,
                    Map.of("sourceSystem", source, "idempotencyKey", message.idempotencyKey()));
        }
        UUID id = UUID.randomUUID();
        jdbc.update("""
                INSERT INTO safety_security.intrusion_inbox_messages (id,source_system,idempotency_key,
    ...
    private void requireAllowlisted(String source) {
        if (!allowlistedSources.contains(source)) {
            throw new IntrusionException(IntrusionErrorCode.INTRUSION_SOURCE_NOT_ALLOWED,
    ...
    private void requireValidSignature(String source, Instant signedAt, String rawPayload, String signature) {
        ...
        if (signedAt == null || Duration.between(signedAt, clock.instant()).abs().compareTo(SIGNATURE_WINDOW) > 0) {
            throw new IntrusionException(IntrusionErrorCode.INTRUSION_INVALID_SIGNATURE,
    ...
    private static String normalise(String source) {
        if (source == null || source.isBlank()) {
            throw new IntrusionException(IntrusionErrorCode.INTRUSION_SOURCE_NOT_ALLOWED, Map.of("sourceSystem", ""));
        }
