...
/** Produces the {@link ActorContext} every S162 command/query is authorised against - mirrors
 * {@code AccessControlActorResolver}; each module in this service keeps its own copy of this plumbing. */
@Component
public class IntrusionActorResolver {
    ...
    /** A system actor for the inbound integration endpoints - a panel or monitoring-feed message is
     * authenticated by HMAC (see {@code IntrusionIntegrationInbox}), not by an SFL role, and never
     * carries one - mirrors S160a's {@code AccessControlActorResolver#resolveIntegrationActor}. */
    ...
    private Set<SflRole> realmRoles(Jwt jwt) {
        return OidcRoleClaims.roleNames(jwt, rolesClaim).stream().map(IntrusionActorResolver::toRole)
    ...
    private static Set<SflRole> parseRoles(String header) {
        return parseCsv(header).stream().map(IntrusionActorResolver::toRole)
                .filter(Objects::nonNull).collect(Collectors.toUnmodifiableSet());
    }
