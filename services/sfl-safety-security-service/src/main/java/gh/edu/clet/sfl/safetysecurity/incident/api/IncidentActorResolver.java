...
/**
 * Produces the {@link ActorContext} every S163 command/query is authorised against, from the
 * OIDC/JWT principal in production or the {@code X-SFL-*} development headers otherwise. Own copy
 * per module, mirroring {@code VisitorActorResolver} rather than reusing it directly - the same
 * reasoning applies: each module in this service keeps its own copy of this small platform-wide
 * plumbing rather than sharing a module-specific-looking class.
 */
@Component
public class IncidentActorResolver {
    ...
    private Set<SflRole> realmRoles(Jwt jwt) {
        return OidcRoleClaims.roleNames(jwt, rolesClaim).stream().map(IncidentActorResolver::toRole)
    ...
    private static Set<SflRole> parseRoles(String header) {
        return parseCsv(header).stream().map(IncidentActorResolver::toRole)
                .filter(Objects::nonNull).collect(Collectors.toUnmodifiableSet());
    }
