...
/** Produces the {@link ActorContext} every S161 command/query is authorised against - mirrors
 * {@code AccessControlActorResolver}; each module in this service keeps its own copy of this plumbing. */
@Component
public class CctvActorResolver {
    ...
    /** A system actor for the CCTV integration endpoint - a VMS message is authenticated by HMAC (see
     * {@code CctvIntegrationInbox}), not by an SFL role, and never carries one. */
    ...
    private Set<SflRole> realmRoles(Jwt jwt) {
        return OidcRoleClaims.roleNames(jwt, rolesClaim).stream().map(CctvActorResolver::toRole)
    ...
    private static Set<SflRole> parseRoles(String header) {
        return parseCsv(header).stream().map(CctvActorResolver::toRole)
                .filter(Objects::nonNull).collect(Collectors.toUnmodifiableSet());
    }
