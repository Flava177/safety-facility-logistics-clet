...
-- The owning deployable is `sfl-safety-security-service`, not the retired
-- `sfl-emergency-notification-service`. The row names the process that runs the migrations, so
-- leaving the old name here would have this schema assert the existence of a service nobody can
-- start. The bounded context is unchanged: S174 is still SFL.SSEMP / Emergency Communications, and
-- it still owns this schema alone.
INSERT INTO emergency_notification.service_metadata (service_name, boundary_description)
VALUES ('sfl-safety-security-service',
