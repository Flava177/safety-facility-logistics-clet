 ...
 * the class Javadoc on {@link AmqpFacilitiesEventTransport} for why a bare {@code send} was not enough.
 *
 * <p>{@code RabbitTemplate} is mocked rather than run against a broker: what is under test here is the
 * transport's reaction to the {@link CorrelationData} future, not RabbitMQ itself. The
 * {@code convertAndSend} call is stubbed to resolve (or not resolve) that future exactly the way
 * {@code PublisherCallbackChannelImpl} does, on the same thread, so the transport cannot tell the
 * difference.
 */
class AmqpFacilitiesEventTransportTest {
    ...
    void an_acknowledged_publish_with_no_return_succeeds() {
        ...
        AmqpFacilitiesEventTransport transport =
                new AmqpFacilitiesEventTransport(rabbitTemplate, "sfl.events", SHORT_TIMEOUT);
    ...
    void a_returned_unroutable_message_is_not_marked_sent_even_though_the_broker_acked_it() {
        ...
        ReturnedMessage returned = new ReturnedMessage(
                org.springframework.amqp.core.MessageBuilder.withBody(new byte[0]).build(), 312,
                "NO_ROUTE", "sfl.events", "ifimp.work-order-created.v1");
    ...
    /**
     * Stubs {@code convertAndSend} to settle the {@link CorrelationData} the way
     * {@code PublisherCallbackChannelImpl} does: {@code setReturned} first (if any), then complete the
     * confirm future - so the transport's ordering assumption is exercised, not assumed.
     */
    ...
    private static OutboxMessage message() {
        return new OutboxMessage(java.util.UUID.randomUUID(), "sfl.ifimp.work-order-created.v1", 1, 1,
                "WorkOrder", "wo-1", "MAIN", "corr-1", "cause-1", null, "{\"probe\":true}");
    }
}
