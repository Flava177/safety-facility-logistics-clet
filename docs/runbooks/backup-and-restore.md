...
Repeat per **schema**, not per service - see the map in [`README.md`](README.md). Two services own two
schemas each (`safety_security` + `emergency_notification`, `fleet_logistics` + `asset_visibility`),
and each is a separate `-n` dump because each is a separate bounded context with its own outbox and
its own audit chain. Dumping a whole database instead would work and would also quietly couple two
contexts' recovery points together, which is the thing the schema split exists to prevent.

There is no cross-service consistent snapshot and there does not need to be:
