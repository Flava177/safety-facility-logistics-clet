...
- *Serialization or routing rejection* - the event name may not match the catalogue. Since 31 July 2026
  both facilities and AVAMP validate the name at the write path (`ServiceEventType`), so
...
**Facilities, S174 and AVAMP have no replay endpoint yet.** Requeue by hand, in a transaction, and
record what you did and why. Note the schema in the `UPDATE`: S174's outbox is
`emergency_notification.outbox_messages` and AVAMP's is `asset_visibility.outbox_messages`, each in
its host service's database rather than in a database of its own. Getting the schema wrong now
targets a table that exists and belongs to somebody else, where it used to target a database that was
not connected - a quieter mistake than the old one, so qualify the name:
