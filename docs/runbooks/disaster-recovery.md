...
|---|---|---|
| 1 | PostgreSQL instances (all three) | Nothing starts without its own schemas |
| 2 | RabbitMQ, if the deployment uses it | Drainers back off cleanly if it is late, but events queue in the outbox meanwhile |
| 3 | `sfl-facilities-service` | S152 is the estate register S153, S159 and readiness all hang off |
| 4 | `sfl-fleet-logistics-service` | Brings AVAMP-Lite up with it - the reference layer the others point at - then fleet, fuel and dispatch |
| 5 | `sfl-safety-security-service` | Brings S174 up with it |
| 6 | `frontend/sfl-operations-ui` | Last: it fails open on permissions, so bringing it up before the services answer makes every gated control **disappear** rather than error |

**The order changed on 5 August 2026 and got simpler, not just shorter.** AVAMP used to be started
first, before facilities, because it is the reference layer. It now starts *with* fleet, since they
are one process - so the ordering constraint it used to impose has become an ordering constraint you
cannot violate. The same is true of S174 and safety-security. Two of the five sequencing decisions
this table used to encode are now structural.
...
All three, per service - and for the two that own two schemas, check both:
