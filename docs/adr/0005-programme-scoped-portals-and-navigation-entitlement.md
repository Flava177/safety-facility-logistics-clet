...
| **SFL.SSEMP** - emergency communications | S174 Emergency mass notification | `sfl-safety-security-service` *(split by ADR 0004, folded back 5 Aug 2026)* |
...
| **SFL.AVAMP** - asset & device visibility | *(cross-cutting reference layer for all 13, not one of the 13)* | `sfl-fleet-logistics-service`, schema `asset_visibility` |
...
itself is correct: it addresses `sfl-safety-security-service` on its own port through the
...
## Amendment, 6 August 2026 - the grain is capability, and the origin is part of the question

- Status: **Accepted and implemented.** Extends this record and **supersedes two things in it**: the
  fail-open resolution of `/actor/permissions` described in *A third grain*, and the assumption
  throughout that one origin serves every programme.

### What was wrong, in one sentence

Every account looked the same.

A driver signed in and saw the estate, the courier manifests, the audit register and the emergency
screens. Not because entitlement was bypassed - the three grains above were all working - but because
the fourth question was never asked. Programme said *FTLMP*. System said *S166, S168*. And then the
item filter asked `FLEET_VEHICLE_READ`, which a driver holds, so the vehicle register was offered.
So did every other screen in the sidebar, because **nearly every role holds nearly every read.**

That is not an oversight in a mapping. It is the wrong predicate. `*_READ` is what you may look at;
it is not what makes a screen yours. `FacilitiesPermissionMatrix` puts fifteen reads in one shared
`READ_ONLY` set precisely so that a technician can see the site a work order is on - and gating
navigation on that set means a technician, a requester, a contractor and a director are offered the
same sidebar. The document generated from the matrices makes this visible at a glance: see the
`Can see` column in [`docs/ROLE_MATRIX_DRAFT.md`](../ROLE_MATRIX_DRAFT.md), which is the same size for
almost everybody, and the `Can do` column, which is not.

### The decision

**Navigation is gated on capability, not on read.** A `NavItem` declares the permission that makes
that screen the actor's work - `FLEET_TRIP_ACKNOWLEDGE`, `FACILITIES_WORK_ORDER_UPDATE`,
`FUEL_LOGBOOK_SUBMIT` - and is offered only to an actor who holds it. Thirty-nine items carry one.

Two things follow that are not obvious and are not concessions:

- **Reviewers are a capability, not an exception.** For an auditor, reading *is* the job, and a
  sidebar that offered them nothing because they change nothing would be a worse answer than the
  read-gated one it replaced. `actorIsReviewer()` treats holding any of eleven audit, evidence-export
  and report-export permissions as itself a capability. That list is enumerated in code, not inferred
  from a name pattern, because "looks like an audit permission" is how the previous generation of this
  logic got things wrong.
- **An item without a capability is still offered whenever its section is.** Most screens are not
  anybody's exclusive job, and requiring a capability on all of them would recreate the empty-sidebar
  failure at a fourth level.

### Fail-closed, reversing the earlier decision

*A third grain* above chose fail-open: if the permission endpoint could not be reached, nothing was
narrowed. That was right when the gate was decorative and five items carried a permission. It is wrong
now. With capability gating, an unanswered request would hand the actor the **whole** dashboard - the
one outcome this record exists to prevent - and it would do it silently.

So `loadActorPermissions()` is fail-closed: unknown means nothing is offered, `permits()` returns
`false` while `granted` is null, and `permissionFailure()` returns a sentence the shell renders as a
banner. An empty rail now distinguishes *you are entitled to no programme here* from *the service did
not answer*, because those need different actions from the reader and the previous version showed the
same blank space for both.

The safety this replaces is not lost, because it was never in the front end: every service authorises
every call from the matrices independently. Fail-open protected against a *dashboard* bug by widening
access to screens the service would refuse anyway - which is to say it protected nothing and hid the
outage.

### The origin is part of the entitlement question

The other assumption in this record was that there is one dashboard origin. There are four (ADR 0006's
amendment): each platform service serves the bundle at `/home` on its own port, and the portal serves
it on 8090.

A bundle therefore has to know **where it is running**, and it asks: `loadServingPlatform()` reads
`GET /api/v1/system/info` relative to its own origin at boot. A platform service answers with its own
code; the portal answers `ALL`. `entitledSections()` then filters on `servesPlatform(section.programme)`
*before* it asks about roles at all, so the facilities service does not offer fleet screens to a fleet
manager who is entitled to them - they are simply not here. `RequireEntitlement` gained a third refusal
panel, `NotServedHere`, which says that rather than implying the actor lacks permission.

**The sign-in page is scoped by the same rule, and this is the part that mattered most to operators.**
`accountsForServingPlatform()` lists only the accounts whose roles are entitled to a programme this
origin serves - thirteen on 8093, twelve on 8091, eight on 8092, all twenty-two on the portal - and
`signIn()` refuses a foreign account with `wrong-platform` **before** it checks the password. Listing
twenty-two accounts on every service read as an invitation and was not one: signing into facilities as
a driver produced a valid session, an empty dashboard, and no explanation anywhere on the page.

Programme entitlement rather than permissions decides that filter, deliberately: permissions are
resolved from the service *after* a session exists, and this decision has to be made before one does.
The two agree at that grain - a role entitled to no programme on a platform holds no permission on it
either - which is checkable in the generated matrix.

### Consequences

- The build-time role fallback is gone. `config.ts` used to supply seven default roles when
  `VITE_SFL_ROLES` was unset, so a production bundle with no configuration granted a broad actor to
  anybody who loaded it. `developmentFallbackActor()` now returns empty strings, and the production
  actor variables are out of `.env.production` entirely.
- Server-side enforcement needed no new machinery, and that is worth recording because the obvious
  reading of "gate the UI on capabilities" is that the API must be taught to match. It already was:
  `FacilitiesAuthorization`, `EmergencyAccessPolicy` and their peers already call the matrices and
  audit the refusal. Coverage measured at the time: 13 of 15 facilities command paths, 5 of 5
  emergency, 17 of 18 fleet - the three gaps being scheduled sweeps that run with no actor. An
  interceptor and an annotation were built to close a gap that did not exist, and removed.
- `authPaths.login` is `/login`, not a platform-prefixed path, so every origin's sign-in is at the
  same place under its own basename.
- Four tests hold this: `navigationPermissions.test.ts`, `platformScoping.test.ts`,
  `accounts.test.ts`, and `DashboardPermissionNamesTest` in `sfl-service-common`, which checks that
  every permission name the bundle asks for is a real `SflPermission`. That last one is in Java rather
  than TypeScript because reading source files from a browser test pulls `node:fs` into a tsconfig
  that has no Node types, and eight compile errors is a high price for one assertion.
