...
Four schemas still need the same migration: `fleet_logistics`, `asset_visibility`,
`safety_security` and `emergency_notification`. The mechanism is shared and already on their
classpath; what each needs is the policy migration and a configuration class binding the GUC to its
own actor resolver. Facilities is the reference to copy.

**Counted in schemas, not services, and that now matters.** Consolidating to three deployables on
5 August 2026 did not reduce this work - RLS policies attach to tables, so folding S174 and AVAMP
into host services left the same four schemas outstanding. It did change one thing: `sfl_app` is a
cluster-wide role, so a service hosting two schemas must grant it usage on both, and a policy
migration that covers only the host's default schema will leave the other silently unprotected while
the service starts perfectly. The facilities V14 loop is catalogue-driven for exactly this reason -
copy that property, not just the file.
