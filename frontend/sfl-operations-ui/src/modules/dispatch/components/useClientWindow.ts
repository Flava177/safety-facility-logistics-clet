...
/**
 * Paging over the window the dispatch service returned, because the dispatch service does not page.
 *
 * Every dispatch collection returns a bare `List<T>` capped by `size`; there is no `page`, no
 * `totalElements` and no `PageResponse` envelope. Two things follow, and both are the point of this
 * hook.
 *
 ...
 * truncated — a table reading "1–25 of 200" while the site holds four hundred items is the kind of
 * thing an operator discovers when a consignment is missing. `truncated` is what the screens use to
 * say so.
 *
 * The fuel module had exactly this and no longer needs it: its collections were moved to a real
 * paged envelope. When the dispatch endpoints follow, this goes the same way — see gap 1 in
 * `docs/dispatch/S171_Dispatch_Frontend_Gap_Register.md`.
 */
